...
tags: [pwnable, ctf, double free, fastbin dup into stack]
---
...
```
이렇게 fake chunk header로 활용할 수 있는 영역이 `0x7fffffffdc15`와 `0x7fffffffdc25` 두 군데가 있는데, 이상하게 `fd`를 `0x7fffffffdc15`로 설정할 경우는 `malloc()`에 실패하고, `0x7fffffffdc25`로 설정할 경우는 `malloc()`에 성공한다.
